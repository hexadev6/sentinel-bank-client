        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mt-10 justify-between items-center">
          {accountType.map((item) => (
            <div className="flex flex-col h-full group ">
              <Card className="rounded-none border shadow-none flex-1 grid grid-cols-1  group-hover:grid-cols-2 relative justify-between  items-center ">
                ...
                <CardBody className="bg-gray-300 flex w-min flex-wrap h-full right-0  absolute top-0 opacity-0 group-hover:opacity-100 flex-col justify-center -translate-x-10 group-hover:translate-x-0 -z-1 transition-all ease-in duration-300">
                  {item.type.map((type) => (
